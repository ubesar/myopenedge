...
const MomentumDayChart = ({ date, bars, symbol, ibHigh, ibLow, momentum, trades, availableDates, selectedDate, onDateChange }: MomentumDayChartProps) => {
  ...
  const entrySet = new Set(trades.map(t => t.entryTime));
  const exitSet = new Set(trades.map(t => t.exitTime));

  const badge = momentum === "bullish"
    ? { text: "🟢 bullish", cls: "bg-emerald-500/15 text-emerald-400" }
    : momentum === "bearish"
    ? { text: "🔴 bearish", cls: "bg-red-500/15 text-red-400" }
    : { text: "⚪ neutral", cls: "bg-muted text-muted-foreground" };
    ...
    <div className="rounded-lg border border-border/30 bg-card/40 backdrop-blur-md p-3 sm:p-4 shadow-lg">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-3 gap-2">
        ...
        <div className="flex items-center gap-3 text-[10px] sm:text-xs">
          <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-full bg-blue-500" />entry</span>
          <span className="flex items-center gap-1"><span className="inline-block h-3 w-3 rounded-full bg-orange-500" />exit</span>
          <span className="flex items-center gap-1"><span className="inline-block h-2 w-4 border border-dashed border-yellow-500/60" />IB range</span>
        </div>
      </div>

      <div className="flex gap-3 h-[260px] sm:h-[360px]">
        <div style={{ width: `min(70%, ${bars.length * 14 + 80}px)`, height: "100%", flexShrink: 0 }}>
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={bars} barCategoryGap={0} margin={{ top: 10, right: 10, bottom: 0, left: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" vertical={false} />
              <XAxis dataKey="time" tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10 }} axisLine={{ stroke: "hsl(var(--border))" }} tickLine={false} interval={tickInterval} />
              <YAxis domain={[domainMin, domainMax]} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 10 }} axisLine={false} tickLine={false} orientation="right" tickFormatter={v => v.toFixed(2)} width={60} />
              <Tooltip
                  ...
                  return (
                    <div className="rounded-md border border-border bg-card px-3 py-2 text-xs shadow-lg">
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                        <span className="text-muted-foreground">O</span><span className="text-right text-card-foreground">{d.open.toFixed(2)}</span>
                        <span className="text-muted-foreground">H</span><span className="text-right text-card-foreground">{d.high.toFixed(2)}</span>
                        <span className="text-muted-foreground">L</span><span className="text-right text-card-foreground">{d.low.toFixed(2)}</span>
              ...
              />
              {/* IB range reference lines */}
              <ReferenceLine y={ibHigh} stroke="hsl(45,90%,50%)" strokeDasharray="6 3" strokeWidth={1} strokeOpacity={0.6}
                label={{ value: `IB H ${ibHigh.toFixed(2)}`, position: "right", fill: "hsl(45,90%,50%)", fontSize: 9 }} />
              <ReferenceLine y={ibLow} stroke="hsl(45,90%,50%)" strokeDasharray="6 3" strokeWidth={1} strokeOpacity={0.6}
                label={{ value: `IB L ${ibLow.toFixed(2)}`, position: "right", fill: "hsl(45,90%,50%)", fontSize: 9 }} />
              ...
              <Bar dataKey="high" fill="transparent" isAnimationActive={false} barSize={10}
                shape={(props: any) => {
                  ...
                  return (
                    <g>
                      {isEntry && <circle cx={wickX} cy={toY(high) - 8} r={4} fill="#3b82f6" stroke="#1d4ed8" strokeWidth={1} />}
                      {isExit && <circle cx={wickX} cy={toY(low) + 8} r={4} fill="#f97316" stroke="#c2410c" strokeWidth={1} />}
        ...
        <div className="flex-1 min-w-[180px] rounded-md border border-border/30 bg-muted/20 p-3 overflow-y-auto hidden sm:block">
          ...
          <div className="space-y-2 text-[11px]">
            <div>
              <span className="text-muted-foreground">trades: </span><span className="font-semibold text-card-foreground">{trades.length}</span>
              <span className="text-muted-foreground ml-2">wins: </span><span className="font-semibold text-emerald-400">{dayWins}</span>
              <span className="text-muted-foreground ml-2">losses: </span><span className="font-semibold text-red-400">{trades.length - dayWins}</span>
            </div>
            <div>
              <span className="text-muted-foreground">day p&l: </span>
              <span className={`font-bold ${dayPnl >= 0 ? "text-emerald-400" : "text-red-400"}`}>{dayPnl >= 0 ? "+" : ""}{dayPnl.toFixed(2)} pts</span>
            </div>
            <div className="border-t border-border/30 pt-2 space-y-1.5">
              ...
              {trades.length === 0 && <p className="text-muted-foreground text-[10px]">no trades on this day</p>}
            </div>
          </div>
        </div>
      </div>
