...
const MomentumChart = ({ title, total, bullish, bearish, choppy }: MomentumChartProps) => {
  ...
  return (
    <div className="rounded-lg border border-border/30 bg-card/40 backdrop-blur-md p-3 sm:p-3 min-w-0 shadow-lg flex flex-col h-full min-h-[220px] sm:min-h-0">
      ...
      <div className="flex gap-2 mt-1.5 shrink-0">
        <div className="flex-1 rounded border border-emerald-500/30 bg-emerald-500/10 px-2 py-1.5 text-center">
          <div className="text-[10px] sm:text-[9px] text-emerald-400 font-medium">Bullish</div>
          <div className="text-base sm:text-sm font-bold text-emerald-400">{bullish}</div>
        </div>
        <div className="flex-1 rounded border border-red-500/30 bg-red-500/10 px-2 py-1.5 text-center">
          <div className="text-[10px] sm:text-[9px] text-red-400 font-medium">Bearish</div>
          <div className="text-base sm:text-sm font-bold text-red-400">{bearish}</div>
        </div>
        <div className="flex-1 rounded border border-yellow-500/30 bg-yellow-500/10 px-2 py-1.5 text-center">
          <div className="text-[10px] sm:text-[9px] text-yellow-400 font-medium">Choppy</div>
          <div className="text-base sm:text-sm font-bold text-yellow-400">{choppy}</div>
        </div>
      </div>
    </div>);
